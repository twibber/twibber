...
>
    ...
    <div class="flex flex-row justify-between items-center w-full bg-gray-900 border-b border-gray-800 p-4">
        <div class="flex flex-col">
            <h1 class="text-lg md:text-xl font-bold flex flex-row justify-start items-center gap-1 w-32 truncate">{data?.profile?.user?.display_name}
                {#if data?.profile?.user?.admin}
                    <Icon icon="material-symbols:verified" class="w-5 h-5 text-yellow-500"/>
                {:else if data?.profile?.user?.verified_person}
            ...
            <div class="mt-2 w-full">
                {#if data?.profile?.user?.counts}
                    <div class="flex flex-wrap justify-start md:justify-around text-xs gap-2 mt-2 w-24 sm:w-48 md:w-full">
                        <div>
                            <span class="font-bold">{data?.profile?.user?.counts?.followers}</span>
                            <span class="text-gray-400">Followers</span>
                        </div>
                        <div>
                            <span class="font-bold">{data?.profile?.user?.counts?.following}</span>
                            <span class="text-gray-400">Following</span>
                        </div>
                        <div>
                            <span class="font-bold">{data?.profile?.user?.counts?.posts}</span>
                            <span class="text-gray-400">Posts</span>
                        </div>
                        <div>
                            <span class="font-bold">{data?.profile?.user?.counts?.likes}</span>
